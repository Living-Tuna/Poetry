    ...
    <div className="min-h-full px-4 py-6 max-w-2xl mx-auto animate-fade-in">
      ...
      {myPoems.length === 0 ? (
        <div className="rounded-xl p-8 text-center" style={{ backgroundColor: 'var(--tp-surface)', border: '1.5px dashed var(--tp-border)' }}>
          <p className="text-sm" style={{ color: 'var(--tp-text-secondary)' }}>{t('writings.empty')}</p>
          <button onClick={onNewPoem}
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={(e) => { e.stopPropagation(); navigateToPoem(p) }}
                    className="p-1.5 rounded-lg transition-colors hover:opacity-70" style={{ color: 'var(--tp-text-secondary)' }} aria-label={t('common.view')}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onEditPoem(p) }}
                    className="p-1.5 rounded-lg transition-colors hover:opacity-70" style={{ color: 'rgba(255,255,255,0.5)' }} aria-label={t('common.edit')}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onDeletePoem(p.id) }}
                    className="p-1.5 rounded-lg transition-colors hover:opacity-70" style={{ color: '#ef4444' }} aria-label={t('common.delete')}>
